...
- **Run window detection** — `cardDanger` detects when opponents are building runs (picked near-adjacent ranks in same suit). +60 danger for gap-filling discards, +30 for extensions. Shark/Mastermind/Nemesis only.
- **Hand reading** — `inferOpponentNeeds` builds per-opponent suit/rank profiles from pick/discard history. Discards matching opponent needs are penalized. Shark/Mastermind/Nemesis only.
- **Predictive go-down** — `aiShouldGoDownHard` bails immediately if 2+ opponents have laid down with ≤3 cards, or if holding 80+ points with any opponent down. Mastermind only.
- **Race mode** — Post-lay-down: when any opponent has ≤3 cards and has laid down, skips cooperative discarding and sheds highest-point cards. `aiFindLayOff` prioritizes highest-point cards in race mode. Shark/Mastermind/Nemesis only.
- **Round-aware strategy** — `getRoundStrategy` returns suit/rank weights per round type. Set-heavy rounds: suited neighbors cheaper to discard. Run-heavy rounds: rank duplicates and off-suit cards cheaper. Shark/Mastermind/Nemesis only.
