...
export default function PlayerProfileModal({ playerId, onClose }: Props) {
  ...
  const winStreak = (() => {
    ...
    for (const g of playerGamesAsc) {
      if (isWinner(g)) { cur++; max = Math.max(max, cur) } else cur = 0
    }
    return max
  })()
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-4 pt-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  { label: 'Wins', value: wins, color: '#8b6914' },
                  { label: 'Avg', value: avgScore, color: '#2c1810' },
                  { label: 'Best', value: bestGame, color: '#2d7a3a' },
                  { label: 'Win %', value: `${winRate}%`, color: '#7c3aed' },
              ...
              <div className="card p-3">
                ...
                <div className="flex flex-col gap-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-[#2d7a3a] font-semibold">
                      {bestGame} pts {bestGameObj ? `· ${fmtDate(bestGameObj.date)}` : ''}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#8b7355]">Worst game</span>
                    <span className="font-mono text-[#b83232] font-semibold">
                      {worstGameObj ? `${getScore(worstGameObj)} pts · ${fmtDate(worstGameObj.date)}` : '—'}
                    </span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="card overflow-hidden">
                ...
                {playerGames.map((g, i) => {
                  ...
                  return (
                    <div key={g.id} className={`flex items-center justify-between px-3 py-2.5 text-sm ${i > 0 ? 'border-t border-[#e2ddd2]/40' : ''} ${i % 2 !== 0 ? 'bg-[#efe9dd]/40' : ''}`}>
                      <div>
